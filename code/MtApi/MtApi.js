 ...
 *                   .title             String
 */
MtApi.prototype.getRecentPostTitles = function(blogid, username, password, numberOfPosts) {
 ...
 */
MtApi.prototype.getCategoryList = function(blogid, username, password) {
   var usr = root.blogger.getUser(username, password);
   var blog = root.blogger.getBlog(blogid.toString());
   if (!blog)
      throw("Couldn't find the blog " + blogid);
 ...
 */
MtApi.prototype.getPostCategories = function(postid, username, password) {
   var usr = root.blogger.getUser(username, password);
 ...
 */
MtApi.prototype.setPostCategories = function(postid, username, password, categories) {
   var usr = root.blogger.getUser(username, password);
   var entry = Story.getById(postid);
   if (!entry)
      throw("Couldn't find the story with id " + postid);
 ...
 */
MtApi.prototype.supportedMethods = function() {
   var result = [
      "metaWeblog.getPost",
      "metaWeblog.newPost",
      "metaWeblog.editPost",
 ...
 */
MtApi.prototype.supportedTextFilters = function() {
   return [];
};
 ...
 *  Antville currently just returns an empty array
 */
MtApi.prototype.getTrackbackPings = function() {
   return [];
};
 ...
 *  @return Boolean true if successful
 */
MtApi.prototype.publishPost = function(postid, username, password) {
   var usr = root.blogger.getUser(username, password);
   var entry = Story.getById(postid);
   if (!entry)
      throw("Couldn't find the story with id " + postid);
